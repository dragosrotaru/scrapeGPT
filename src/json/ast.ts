...
export const generateAST = (json: JSONType, ast?: JSONAST): JSONAST => {
    ...
    if (isJSONObject(json)) {
        ...
        if (ast.kind === "union") {
            const existing = ast.values.find(
                (v) =>
                    v.kind === "object" && objectsHaveSameKeys(v.values, json)
            );
            if (existing) {
                ast.values = [
                    ...ast.values.filter(
                        (v) =>
                            v.kind !== "object" ||
                            !objectsHaveSameKeys(v.values, json)
                    ),
                    generateAST(json, existing),
                ];
                return ast;
    ...
    if (isJSONArray(json)) {
        ...
        if (!ast) {
            ...
            let local: JSONAST = generateAST(json[0] as JSONType);
            for (let i = 1; i < json.length; i++) {
                const val = json[i];
                local = generateAST(val as JSONType, local);
            }
            return {
                kind: "array",
                values: [local],
